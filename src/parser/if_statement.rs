...
use super::{ASTNode, statement_list::StatementList, expression::Expression, generator::Generator, scope::Scope};

#[derive(Debug)]
pub struct IfStatement {
    statements: Rc<StatementList>,
    condition: Rc<Expression>,
    else_part: Option<Rc<StatementList>>
}

impl ASTNode for IfStatement {
    fn parse(lexer: &mut crate::lexer::Lexer, scope: &mut Scope) -> Result<Rc<Self>, LexerError> where Self: Sized {
        lexer.expect(Token::IF)?;
        lexer.expect(Token::LPAREN)?;
        let condition = Expression::parse(lexer, scope)?;
        lexer.expect(Token::RPAREN)?;
        let statements = StatementList::parse(lexer, scope)?;
        let mut else_part: Option<Rc<StatementList>> = None;
        if lexer.peek() == Token::ELSE {
            lexer.next();
            else_part = Some(StatementList::parse(lexer, scope)?);
        }
        Ok(Rc::new(IfStatement { statements: statements, condition: condition, else_part: else_part }))
    }
